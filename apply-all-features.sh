...
#
# Merges ALL feature branches from dots-hyprland-dev into a single integration
# branch, then deploys the merged config files to the live ~/.config directory.
#
...
#   2. Sequentially merges all feature branches (dependency-aware order)
#   3. Auto-resolves known conflicts (READMEs, keybinds)
#   4. Backs up your current live config
#   5. Copies the merged dotfiles into ~/.config, ~/.local/share, etc.
#   6. Copies overlay files (dots/quickshell/) on top
#   7. Optionally installs the AI assistant wake-word service
#   8. Reloads Hyprland
#
# Usage:
...
INTEGRATION_BRANCH="integration/all-features-${TIMESTAMP}"
...
# ─── Feature branches in merge order (dependency-aware) ────────────────────────
#
# Order rationale:
...
#
# LOCAL-ONLY branches (not on GitHub remote):
#   - feature/overview-monitor-binding-dropdown
#   - feature/nova-email-todo-open
...
if ! git diff --quiet HEAD 2>/dev/null; then
    ...
    git stash push -m "apply-all-features auto-stash ${TIMESTAMP}"
    STASHED=true
else
    STASHED=false
fi
...
log "All ${#FEATURE_BRANCHES[@]} feature branches verified"
...
step "Step 2: Merge feature branches (${#FEATURE_BRANCHES[@]} branches)"
...
echo -e "
${GREEN}${BOLD}All features have been applied!${RST}

${CYAN}Features installed:${RST}
  ✓ WiFi reconnect fix
  ✓ MPRIS active player fix (media controls)
  ✓ GitHub Copilot integration (AI panel)
  ✓ Custom keybinds & scripts (Docker, VPN, proxy toggle)
  ✓ US date format & world clocks (sidebar)
  ✓ Home Assistant integration (bar panel)
  ✓ Overview monitor binding dropdown
  ✓ GPU/NPU monitoring (resource bar)
  ✓ VPN status indicator (bar)
  ✓ AI assistant aurora glow effect
